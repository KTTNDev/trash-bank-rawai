...
export default function LandingPage() {
  ...
  const quickLinks = [
    { name: "หน้าหลักรวมบริการ", url: "https://e-service-rawai-center.vercel.app/", color: "bg-blue-50" },
    ...
    { name: "Traffy Fondue", url: "https://landing.traffy.in.th?key=elqOlHUe", color: "bg-orange-50" },
    { name: "ระบบ E-Office", url: "https://rawai.s.eoffice.go.th/portal/home", color: "bg-indigo-50" },
    { name: "ศูนย์บริการ OSS", url: "https://www.dla.go.th/land/oss.do", color: "bg-blue-50" }
  ];
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 overflow-x-hidden selection:bg-emerald-100">
      ...
      <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end gap-4">
        {isMenuOpen && (
          <div className="mb-2 w-64 bg-white/90 backdrop-blur-2xl rounded-[2.5rem] border border-white shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-300 text-left">
            <div className="p-6 bg-slate-900/5 border-b border-slate-100">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Rawai Sabai Services</p>
      ...
      {/* 3. Hero Section: 🟢 พร้อมลายน้ำภาพพื้นหลัง 🟢 */}
      <section className="pt-28 pb-12 md:pt-44 md:pb-24 px-4 md:px-6 relative">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div className="space-y-8 text-center lg:text-left relative">
            {/* 🟢 Mobile Watermark: ภาพพื้นหลังจางๆ 🟢 */}
            <div className="absolute inset-0 -top-10 flex items-center justify-center lg:hidden opacity-[0.08] pointer-events-none -z-10 scale-150 rotate-12 overflow-hidden">
               <img src="https://scontent.furt1-1.fna.fbcdn.net/v/t39.30808-6/634212959_1256933013202535_2453466340996406974_n.jpg?stp=cp6_dst-jpg_tt6&_nc_cat=106&ccb=1-7&_nc_sid=7b2446&oh=00_AftkC1bBKPHoqhkRMS1OoOF03xc50CpFvbF751iXuMMaag&oe=69A548DD" className="w-full h-auto object-contain" alt="" />
            </div>

            <div className="space-y-6 relative z-10">
              <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest"><Sparkles className="w-3.5 h-3.5" /> Sustainable Rawai Sabai City</div>
              <h1 className="text-5xl md:text-7xl lg:text-8xl font-black leading-[1] tracking-tight text-slate-900">ธนาคารขยะ <br/><span className="text-transparent bg-clip-text italic" style={{ backgroundImage: brandGradient }}>ดิจิทัล</span> ราไวย์</h1>
              <p className="text-slate-500 text-sm md:text-lg font-medium max-w-xl mx-auto lg:mx-0">คืนงบประมาณกำจัดขยะ 20,262.73 ตัน/ปี กลับสู่การพัฒนาตำบลอย่างยั่งยืน</p>
            </div>

            {/* Quick Access Grid (Compact Version) */}
          ...
          <div className="hidden lg:block relative animate-in fade-in slide-in-from-right duration-1000">
             <div className="w-full h-[550px] bg-white rounded-[4rem] overflow-hidden rotate-2 relative shadow-2xl border-[12px] border-white">
                <img src="https://scontent.furt1-1.fna.fbcdn.net/v/t39.30808-6/634212959_1256933013202535_2453466340996406974_n.jpg?stp=cp6_dst-jpg_tt6&_nc_cat=106&ccb=1-7&_nc_sid=7b2446&oh=00_AftkC1bBKPHoqhkRMS1OoOF03xc50CpFvbF751iXuMMaag&oe=69A548DD" className="w-full h-full object-cover opacity-90" alt="Hero" />
             </div>
             <div className="absolute -bottom-8 -left-8 bg-white p-6 rounded-3xl shadow-2xl border border-emerald-50 max-w-[240px] z-10 animate-bounce-slow">
                <ShieldCheck className="text-emerald-500 w-8 h-8 mb-3" /><h5 className="font-black text-slate-800 text-sm">โปร่งใส 100%</h5><p className="text-[10px] font-bold text-slate-400 leading-tight">บันทึกข้อมูลแบบดิจิทัล ตรวจสอบยอดเงินได้ตลอด 24 ชม.</p>
             </div>
          </div>
        </div>
      </section>

      {/* 4. Awareness Section: 🟢 ปรับสูตรคำนวณ 725 บาท/ตัน 🟢 */}
      ...
      {/* 5. Success Stats: 🟢 แก้บั๊กครัวเรือน (Member จริง) 🟢 */}
      <section id="stats" className="py-16 md:py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 md:px-6 text-center">
          <h2 className="text-2xl md:text-3xl font-black mb-12 uppercase tracking-tight">ความสำเร็จของโครงการ</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-8 text-left">
            {loading ? <><div className="col-span-2 md:col-span-1"><SkeletonCard height="h-64"/></div><SkeletonCard height="h-64"/><SkeletonCard height="h-64"/></> : <>
              ...
              <StatItem icon={<BadgeDollarSign />} label="เงินสะสมในระบบ" value={stats?.totalMoney?.toLocaleString() || '0'} unit="บาท" color="text-blue-600" />
              {/* 🟢 อ้างอิง Member จริงแล้วครับ 🟢 */}
              <StatItem icon={<Users />} label="ครัวเรือนที่ร่วมโครงการ" value={stats?.totalMembers || '0'} unit="ราย" color="text-amber-600" />
            </>}
          </div>
        </div>
      </section>

      {/* 6. PR & Price Sections (Skeleton Ready) */}
      <section id="schedule" className="py-20 px-4 md:px-6 bg-white border-t border-slate-50">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
            {loading ? [1, 2, 3].map(i => <SkeletonCard key={i} height="h-64" />) : announcements.map((item: any) => (
              <div key={item.id} className="p-8 bg-slate-50 rounded-[2.5rem] border border-transparent hover:border-emerald-500/20 hover:bg-white hover:shadow-xl transition-all group text-left">
                <div className="flex justify-between items-start mb-6">
                  <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-emerald-600 shadow-sm transition-all"><MapPin className="w-6 h-6" /></div>
      ...
      {/* 7. Footer: Visitor Stats สไตล์ CCTV */}
...
function StatItem({ icon, label, value, unit, color, isMain }: any) {
  return (
    <div className={`bg-white rounded-[2.5rem] md:rounded-[3.5rem] p-6 md:p-10 border border-slate-100 shadow-sm flex flex-col items-center text-center group hover:shadow-xl transition-all h-full ${isMain ? 'md:p-14' : ''}`}>
      ...
      <div className="flex items-baseline gap-1 justify-center"><span className={`${isMain ? 'text-4xl md:text-6xl' : 'text-2xl md:text-5xl'} font-black text-slate-900 tabular-nums`}>{value || '0'}</span><span className="text-[10px] md:text-sm font-bold text-slate-400 uppercase ml-1">{unit}</span></div>
    </div>
  );
}

function SkeletonCard({ height }: { height: string }) {
  return <div className={`${height} w-full bg-slate-200/60 rounded-[2.5rem] animate-pulse relative overflow-hidden`}><div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full animate-shimmer" /></div>;
}